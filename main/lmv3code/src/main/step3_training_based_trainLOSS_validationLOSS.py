...
from src.main.config.prod_mapping import product_code_map, document_code_map
...
log_dir = "src/main/logs"  # Directory to store the TensorBoard logs
train_writer = tf.summary.create_file_writer("src/main/logs/train/")
test_writer = tf.summary.create_file_writer("src/main/logs/test/")
best_train_test_writer= tf.summary.create_file_writer("src/main/logs/best")
...
product_wise_folder.read("src/main/config/prod.ini")
...
best_precision_train=None
best_recall_train=None
best_f1_train=None

best_loss_test=None
best_precision_test=None
best_recall_test=None
best_f1_test=None
...
for epoch in range(num_train_epochs):
    ...
    ####################testing################
    if  best_loss_test is None:
      best_loss_test=val_loss
    if best_precision_test is None:
      best_precision_test= precision
      best_recall_test= recall
      best_f1_test= f1
      
    ########################training######################
    if  best_loss_train is None:
      best_loss_train= loss
    if best_precision_train is None:
      best_precision_train= precision
      best_recall_train= recall
      best_f1_train= f1
          
    if val_loss< best_loss_test and f1 > best_f1_test and recall > best_recall_test: #testing loss
        best_loss_test = val_loss
        best_precision_test = precision
        best_recall_test = recall
        name = "Best_Model_val"
        ...
        print(f'Model is {epoch} saving based on val loss +++++++++++++++++++++++++++++++++')
        ...
        print('+++++++++++++ Test ++++++++++++++++++')
        print(f"best Validation Loss: {best_loss_test}" )
        print("best Precision:", best_precision_test)
        print("best Recall:", best_recall_test)
        model.save_pretrained(os.path.join(folder_path, name))
        
    if loss< best_loss_train and f1 > best_f1_train and recall > best_recall_train: #training loss
        best_loss_train = loss
        best_precision_train = precision
        best_recall_train = recall
        ...
        if not os.path.exists(os.path.join(folder_path, name)):
          os.mkdir(os.path.join(folder_path, name))
        print(f'Model is {epoch} saving based on train loss +++++++++++++++++++++++++++++++++')
        with best_train_test_writer.as_default():
            tf.summary.scalar("Best train loss ", best_loss_train.detach().cpu(), step=epoch)
            # tf.summary.scalar("Best val loss", val_loss.cpu(), step=epoch)
        print('+++++++++++++++== Train ++++++++++++++++++++++==')
        print(f"best Validation Loss: {best_loss_train}" )
        print("best Precision:", best_precision_train)
        print("best Recall:", best_recall_train)
        model.save_pretrained(os.path.join(folder_path, name))        
...
print("Overall results based on test epoch:", val_result)
print(class_report)
with open(os.path.join(folder_path, "test_report_based_test_epoch.txt"), 'w') as f:
...
test_result = val_result
test_all = class_report
print("Overall results based on train epoch:", val_result)
print(class_report)
with open(os.path.join(folder_path, "test_report_based_train_epoch.txt"), 'w') as f:
    f.write(str(val_result))
    f.write('\n')
    f.write(class_report)
f.close()
