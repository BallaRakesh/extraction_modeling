...
label2id = {'OTHERS': 0, 'BOE': 1, 'BOL': 2, 'AWB': 3, 'IC': 4, 'PL': 5, 'CI': 6, 'COO': 7, 'CS': 8}
id2label = {0: 'OTHERS', 1:'BOE', 2:'BOL', 3:'AWB', 4:'IC', 5:'PL', 6:'CI', 7:'COO', 8:'CS'}
...
class CustomData:
    ...
    @classmethod
    def convert_image_mode(cls, img: Image.Image, target_mode: str = 'L') -> Image.Image:
        """
    ...
    @classmethod
    def create_custom_dataset(cls, root_folder: str) -> Dataset:
        """
        ...
        for class_name in sorted(os.listdir(root_folder)):
            ...
            for img_name in os.listdir(class_path):
                ...
                if img_name.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
                    ...
                    # img = cls.convert_image_mode(img, target_mode='L')  # Ensure grayscale mode
...
train_root_folder = "/home/ntlpt19/Downloads/Classification_final_training/V3_ROOT/test"
test_root_folder = "/home/ntlpt19/Downloads/Eval_classification/BILLS"
ocr_gv = '/home/ntlpt19/Downloads/Classification_final_training/OCR_GV'
ocr_tesseract = '/home/ntlpt19/Downloads/Classification_final_training/OCR_GV'


train_dataset_main = CustomData.create_custom_dataset(train_root_folder)
test_dataset_main = CustomData.create_custom_dataset(test_root_folder)
...
output_file = "id2label.txt"  # Specify the output file name
with open(output_file, "w") as file_:
    for id, label in id2label.items():
        file_.write(f"{id}: {label}\n")  # Write each key-value pair on a new line
file_.close()
...
print((Image.open(valid_df['img'][0])).size)
...
custom_pytesseract = False
import ast

def apply_ocr(image, img_name_):
    """
...
#     image = Image.open(image_fp)
    ...
    if custom_pytesseract:
        # print(img_name_)
        file_path_gv = os.path.join(ocr_gv, img_name_[:-4]+'_text.txt')
        # file_path_pytes = os.path.join(ocr_tesseract, img_name_[:-4]+'.json')
        ...
        #         word_coordinates = json.load(file)
        #         file.close()
...
#         original_image = Image.open(image).convert("RGB")
    ...
    if use_ocr == True:
      entries = apply_ocr(image, image_name_)
...
class RVLCDIPData(Dataset_torch):

    def __init__(self, image_list, label_list,image_names_list, target_size, tokenizer, max_len = 512, transform = None):
    ...
    def __getitem__(self, idx):
        ...
        img_loaded = CustomData.convert_image_mode(img_loaded, target_mode='L')  # Ensure grayscale mode
        img_loaded.load()  # Ensure the image is valid
        # print(img)
        
        label = self.label_list[idx]
        image_names_idx = self.image_names_list[idx]
        ...
        final_encoding['image_name'] = image_names_idx
        # final_encoding['image_name'] = [torch.tensor(image_names_idx)]
...
train_ds = RVLCDIPData(train_df['img'].tolist(), train_df['label'].tolist(),train_df['image_names'].tolist(),
                      target_size, tokenizer, config['max_position_embeddings'], transform)
val_ds = RVLCDIPData(valid_df['img'].tolist(), valid_df['label'].tolist(),valid_df['image_names'].tolist(),
                      target_size, tokenizer,config['max_position_embeddings'],  transform)
...
def collate_fn(data_bunch):
  ...
  for i in data_bunch:
    ...
    for key in list(dict_data_bunch.keys()):
        ...
        if key == "image_name":  # Skip or handle strings separately
            pass
            # print(dict_data_bunch[key])
            # dict_data_bunch[key] = [item for item in dict_data_bunch if isinstance(item, str)]
            # dict_data_bunch[key] = [item[key] for item in dict_data_bunch]
        else:
...
label2id_infer = {'AWB': 0, 'BOE': 1, 'BOL': 2, 'CI': 3, 'COO': 4, 'CS': 5, 'IC': 6, 'OTHERS': 7, 'PL': 8}
id2label_infer = {v: k for k, v in label2id.items()}


model_path = '/home/ntlpt19/Downloads/Classification_final_training/docformer_model/model-epoch=02.ckpt'
...
for idx, batch in enumerate(tqdm(pl_dl.val_dataloader())):
    ...
    batch = {k: v.to(device) if k != 'image_name' else v for k, v in batch.items()}

    list_of_image_name.extend(batch['image_name'])
    ...
    print('IMAGE NAME', batch['image_name'][0], 'ACTUAL>>>>', id2label.get(batch['label'].item()), 'PREDICTED>>>>', id2label_infer.get(preds.item()))

    # print(list_of_image_name)
    # print(list_of_gt)
    # print(list_of_pred)
data = {'Image Name': list_of_image_name, 'Ground Truth': list_of_gt, 'Predicted': list_of_pred}
