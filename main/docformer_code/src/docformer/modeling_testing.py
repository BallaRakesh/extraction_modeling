...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000):
        ...
        print(layoutlm_dummy)
        # exit('OLLLLLLLLLL')
        #position_embeddings = nn.Embedding.from_pretrained(layoutlm_dummy.layoutlm.embeddings.position_embeddings.weight)
        ###################################################
        ...
        ##############################################################################
        position_weights = layoutlm_dummy.layoutlm.embeddings.position_embeddings.weight
        # Take only the first max_len positions and convert to buffer
        pe = position_weights[:max_len].unsqueeze(0)  # Shape: (1, max_len, d_model)
...
class DocFormerEmbeddings(nn.Module):
    ...
    def forward(self, x_feature, y_feature):
        ...
        print(f"Min index: {x_feature[:,:,0].min().item()}, Max index: {x_feature[:,:,0].max().item()}")
