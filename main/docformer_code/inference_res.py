...
class RVLCDIPData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        final_encoding['image_path'] = img_path
...
def collate_fn(data_bunch):
  ...
  for key in list(dict_data_bunch.keys()):
      if key == "image_path":  # Skip or handle strings separately
          pass
      else:
...
for idx, batch in enumerate(tqdm(pl_dl.val_dataloader())):
    ...
    batch = {k: v.to(device) if k != 'image_path' else v for k, v in batch.items()}

    list_of_image_name.extend(batch['image_path'])
    ...
    print('IMAGE NAME', batch['image_path'][0], 'ACTUAL>>>>', id2label.get(batch['label'].item()), 'PREDICTED>>>>', id2label_infer.get(preds.item()))
    # print('ACTUAL>>>>', id2label.get(batch['label'].item()), 'PREDICTED>>>>', id2label_infer.get(preds.item()))
...
for idx, batch in enumerate(tqdm(pl_dl.train_dataloader())):
    # move batch to device
    # batch = {k:v.to(device) for k,v in batch.items()}
