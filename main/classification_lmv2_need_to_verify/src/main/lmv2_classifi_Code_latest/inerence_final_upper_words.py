...
encoded_dataset = updated_dataset.map(lambda example: encode_example(example), features=training_features, batched=True, batch_size=1)
...
test_dataloader = torch.utils.data.DataLoader(new_modified_dataset, batch_size=2, shuffle=True)
