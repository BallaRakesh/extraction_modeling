...
Validates on
=============
1) ReviewID 1 = Tarun Sharma on Nov 8
"""
...
import numpy as np
import pandas as pd
from src.main.lmv2_classifi_Code_latest.ocrpipeline import ApplyOcr
...
from src.main.lmv2_classifi_Code_latest.utility import get_logger_object_and_setting_the_loglevel, set_basic_config_for_logging
#########################################################################################################
...
# Configuration: 16G
#####################################################################################
set_basic_config_for_logging(filename=f"classification_training_{datetime.now()}")
logger = get_logger_object_and_setting_the_loglevel()


dataset_path = "/New_Volume/trade-finance/final_delivery/classification_training/V2_Final_Training_Data"
ocr_path = '/New_Volume/trade-finance/final_delivery/classification_training/ocr'
...
training_set, testing_set = train_test_split(datasetall, test_size=0.2, random_state=42, stratify=label_list)
...
def train_test_split_base(dataset, flag):
    if flag=='train':
        ...
        encoded_dataset = new_updated_dataset.map(lambda example: encode_example(example), features=training_features, batched=True, batch_size=2)
