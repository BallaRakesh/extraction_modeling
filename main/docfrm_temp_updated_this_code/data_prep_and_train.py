...
# # Example: Moving to a desired location
# dest_path = "/home/data_science/geo_testing/docformer/ROOT"
# shutil.move(pdavpoojan_the_rvlcdip_dataset_test_path, dest_path)
...
class RVLCDIPData(Dataset):
    ...
    def __getitem_org__(self, idx):
        ...
        for key in keys_to_reshape:
            if final_encoding[key].dim() == 0:
                final_encoding[key] = final_encoding[key].unsqueeze(0)  # Add dimension for scalars
            if final_encoding[key].numel() == 0:
                final_encoding[key] = torch.zeros((self.max_len,))  # Handle empty tensors
            final_encoding[key] = final_encoding[key][:self.max_len]


        # final_encoding['label'] = torch.as_tensor(label).long()
        
        final_encoding['label'] = torch.as_tensor(label).long().unsqueeze(0)  # Make sure it's batch-compatible
        
        print('DONE final_encoding label')
        print(final_encoding)
        print('idx', idx , '>>>>>>>>>>>>>>>>>>>>>>>>>>>')
        print("DEBUG: Checking dimensions before slicing...")
        for key in keys_to_reshape:
            print(f"{key} shape: {final_encoding[key].shape}")
            print(f"{key} content: {final_encoding[key]}")
            if final_encoding[key].dim() == 0:
                print('YES', key)
            if final_encoding[key].numel() == 0:  # Check if tensor is empty
                print('YES', key)
        print('label>>>>>>>>>>>before.', final_encoding['label'])
        # final_encoding['label'] = final_encoding['label'].unsqueeze(0).long()
        final_encoding['label'] = torch.tensor(label).long().unsqueeze(0)
        print('label>>>>>>>>>>> after.', final_encoding['label'])
        
        max_len = max(len(value) for value in final_encoding.values() if isinstance(value, torch.Tensor))
        for key, value in final_encoding.items():
            if isinstance(value, torch.Tensor):
                if value.dim() == 1:
                    final_encoding[key] = value.repeat(max_len)[:max_len]  # Ensure consistent shape
                elif value.size(0) < max_len:
                    # Pad tensors with zeros if necessary
                    pad_size = max_len - value.size(0)
                    padding = torch.zeros((pad_size,) + value.shape[1:], dtype=value.dtype)
                    final_encoding[key] = torch.cat([value, padding], dim=0)
    ...
    def __getitem__(self, idx):
        # Ensure index is within dataset bounds
        if isinstance(idx, list):
            if len(idx) > 1:
                raise ValueError("INDEX LEN ISSUE: Only one index should be passed at a time.")
            idx = idx[0]
        
        # Ensure idx is within the range of image_list
        if idx < 0 or idx >= len(self.image_list):
            raise IndexError(f"Index {idx} is out of bounds for dataset of size {len(self.image_list)}")
        ...
        # Robust handling of feature tensors
        keys_to_reshape = ['x_features', 'y_features', 'resized_and_aligned_bounding_boxes']
        for key in keys_to_reshape:
            if key not in final_encoding:
                print(f"Warning: Key {key} not found in final_encoding")
                final_encoding[key] = torch.zeros(1, self.max_len, dtype=torch.long)
                continue

            tensor = final_encoding[key]
            ...
            if tensor.shape[1] > self.max_len:
                tensor = tensor[:, :self.max_len]
            elif tensor.shape[1] < self.max_len:
                pad_size = self.max_len - tensor.shape[1]
                padding = torch.zeros(
                    tensor.shape[0], 
                    pad_size, 
                    *tensor.shape[2:], 
            ...
            final_encoding[key] = tensor

        # Ensure label is a tensor
        final_encoding['label'] = torch.as_tensor(label).long().unsqueeze(0)

        # Ensure image tensor is 4D (batch, channels, height, width)
        img = final_encoding['resized_scaled_img']
        if img.ndim == 3:
            img = img.unsqueeze(0)  # Add batch dimension
        if img.shape[1] > 3:
            img = img[:, :3, :, :]
        elif img.shape[1] < 3:
            pad_channels = 3 - img.shape[1]
            padding = torch.zeros(
                (img.shape[0], pad_channels, img.shape[2], img.shape[3]), 
                dtype=img.dtype
            )
            img = torch.cat([img, padding], dim=1)
        final_encoding['resized_scaled_img'] = img

        # Robust handling of feature tensors to ensure 2D shape
        keys_to_reshape = ['x_features', 'y_features', 'resized_and_aligned_bounding_boxes']
        for key in keys_to_reshape:
            if key not in final_encoding:
                # Create a placeholder tensor if key is missing
                final_encoding[key] = torch.zeros(1, self.max_len, dtype=torch.long)
                continue
            ...
            # Ensure first two dimensions are batch and sequence length
            if tensor.ndim > 2:
                # Flatten extra dimensions beyond the first two
            ...
            # Ensure at least 2 dimensions for batch and sequence length
            if tensor.ndim == 1:
                tensor = tensor.unsqueeze(0)
            
            final_encoding[key] = tensor

        # Ensure the tensor has at least 2 dimensions before returning
        for key in keys_to_reshape:
            if final_encoding[key].ndim == 1:
                final_encoding[key] = final_encoding[key].unsqueeze(0)
