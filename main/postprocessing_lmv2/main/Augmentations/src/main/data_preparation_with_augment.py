...
def get_ocr_vision_api(image_path):
    os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = "/New_Volume/Rakesh/Trade_Finance_Training/trade-finance-mvp/training/lmv2code/src/main/extraction/spheric-time-383904-f1b421d86eef.json"
...
if __name__== "__main__":
    ...
    folder_path = '/New_Volume/Rakesh/ROOT_AUG'
    #####################################################################
    configur = ConfigParser()
    configur.read('/New_Volume/Rakesh/Trade_Finance_Training/trade-finance-mvp/training/lmv2code/src/main/extraction/traini_valid_utility.ini')
    ...
    if is_augment:
        ...
        ######## data preparation needed to be updated for the agumentation   
    exit('////')
    ...
    for file in tqdm.tqdm(img_files):
        ...
        if os.path.exists(os.path.join(labels_path, file + ".txt")):
            ...
            if not os.path.exists(os.path.join(master_path, file + "_text.txt")):            
                word_coordinates, all_text = get_ocr_vision_api(os.path.join(images_path, 
                                                                             file + ".png"))
            else:
                # it will read
                ocr_already_available_flag  = True
                word_coordinates = json.load(open(os.path.join(master_path, 
