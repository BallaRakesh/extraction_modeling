...
def image_result(file, model, processor, device):
	...
	for i, image in enumerate(ImageSequence.Iterator(im)):
		...
		image_chunk = encoded_inputs['pixel_values'][0]
		image_chunks = list()
		...
		input_dict = {
			...
			'pixel_values': images.float().to(device)
		}
		...
		for i in range(len(all_text)):
			if all_text[i] not in ['[s]', '[/S]']:  # and all_predictions[i] != 'O':
				# print(i)
				if (curr_box != all_boxes[i] and len(temp_text) > 0):
					# print("1: ", all_text[i])
