...
def sanitize_text(current_value):
    sanitized_value = re.sub(r'[\\\/\'"]', '', current_value)
    return sanitized_value

def sanitize_text_2(current_value):
    brace_count = 0
    for character in current_value:
        if character == "}":
            brace_count += 1
    if brace_count > 1:
        return current_value[:current_value.index("}")]
...
def extract_key_value_pairs(input_text):
    ...
    for line in lines:
        ...
        if match:
            ...
            current_value = sanitize_text(current_value)
        elif current_key:
    ...
    if current_key:
        current_value = sanitize_text(current_value)
        current_value = sanitize_text_2(current_value)
        pairs[current_key] = current_value.strip()
...
if __name__ == "__main__":
    ...
    results_folder = '/home/ntlpt19/LLM_training/EVAL/COO/results/text_files'
        ...
        if pd.notna(file_name):
            ...
            if not pd.isnull(row['gt_len']) and not pd.isnull(row['prediction_count']):
                ...
                if os.path.exists(file_path):
                    counter += 1
                    print("The current counter value is:",counter)
                    ...
                    for key, value in json_str.items():
                        if key in output.keys():
                            lst = output[key]
                            del output[key]
                            output[key + '~' + key] = lst + [[value,dummy_bbox,dummy_confidence]] 
                        else:    
                    ...
                    print("The missing file path is:",file_path)