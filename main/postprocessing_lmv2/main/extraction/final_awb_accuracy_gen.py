...
# 	# pattern = r'(?i)AGAINST (PAYMENT|ACCEPTANCE)|DELIVER DOCUMENTS AGAINST : (PAYMENT|ACCEPTANCE)'
# 	pattern = r'(?i)AGAINST (PAYMENT|ACCEPTANCE)|DELIVER DOCUMENTS AGAINST : (PAYMENT|ACCEPTANCE)|non - payment / non - acceptance|against (payment|acceptance) after filter'
...
def doc_delivery_pp(input_string):
    ...
    patterns = [
        r'(against payment / acceptance).*(after filter)',
        r'(?i)AGAINST (PAYMENT|ACCEPTANCE)|DELIVER DOCUMENTS AGAINST : (PAYMENT|ACCEPTANCE)|non - payment / non - acceptance'
    ]
...
# logger.add(f"testing_country{current_datetime}.log", rotation="500 MB", retention="7 days")
...
if __name__ == '__main__':
    ...
    for doc_code_ in doc_code_list:
        ...
        for file, predicted_files in zip(data_files, result_files):
            ...
            #exit()
            #if 'currency_amount' or 'csh_bill_currency' or 'csh_bill_amount' in actual_list:
            # print(labels)
            # exit('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
            actual_amt = []
            actual_curr = []
            ...
            if 'csh_bill_amount' in pred_list:
                ...
                print(predicted)
                # exit("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
                        # 	actual_amt.append(act_amount)
                        # if len(act_currency)>0:
                        # 	actual_curr.append(act_currency)				
                    #actual_curr.append(act_currency)
                    #print(actual_curr)
                    #exit()
                    ...
                    if key in labels and key in predicted:
                        if len(labels[key]) == 1 and len(predicted[key]) == 1:               
                            ...
                            if key in ['doc_delivery_instruction']:
                                pred_before_filter = row[3]
                                act_before_filter = row[2]
                                row[3] = doc_delivery_pp(row[3])
                                row[2] = doc_delivery_pp(row[2])
                                #logger.info(f'before filter => {pred_before_filter} and after filter => {row[3]}')
                            ...
                            if key in ['date_of_invoice', 'lc_date', 'awb_date', 'bill_of_lading_date', 'invoice_date', 'issue_date']: #actual
                            #if key in ['abc']:
                                print(row[2], row[3])
                            ...
                            if key not in ['date_of_invoice', 'lc_date', 'bill_of_lading_date', 'invoice_date', 'issue_date']:
                            ...
                            if key in ["page_no","original_Number","to_place","lc_ref_no","lc_ref_number","consignee_name"]:
                                ...
                                elif key in ['to_place', 'at_place', 'place_of_receipt', 'freight_collect_at']:
                            ...
                            if key in ['country_of_origin_of_goods', 'country_of_final_destination', "drawee_bank_address", "drawer_bank_address","drawee_address","nostro_bank_address", "drawer_address", "consignee_address",'consignee_addres','shipper_address','notify_party_address', "consignor_address", "coo_issuer_address"]:

                                act_val_address = labels[key][0][0]
                                pre_val_address = predicted[key][0][0]
                                # logger.info(f'actual addresses before filter => {act_val_address}')
                                ...
                                #exit()
                                # and after filter => {row[3]}')
                                # logger.info(f'actual addresses after filter => {act_val_address}')
                            ...
                            if key in ['date_of_invoice', 'lc_date', 'bill_of_lading_date', 'invoice_date', 'issue_date']:#, 'invoice_date']:
                            #if key in ['abc']:
                                pred_list = []
                                actual_list = []
                                for i in range(l2):
                            ...
                            if key not in ["drawee_bank_address", 'consignee_addres', 'carrier_country', 'carrier_name', 'agent_country','agent_name', "drawer_address", "drawer_bank_address", "drawer_bank_bottom_address","drawee_address","document_enclosed", "consignee_address","consignor_address", "coo_issuer_address", "description_of_goods", "marks_and_no_of_packages","notify_party_name", 'dimension', 'to_place', 'means_of_transport', 'issue_date', 'date_of_invoice', 'lc_date', 'bill_of_lading_date','invoice_date','issue_date']:
                                flag01 = []							
                                for i in range(l1):
                                    new_row = row.copy()
                            ...
                            '''if len(flag01)>0 and key in ["drawee_bank_address", "drawer_bank_address", "drawer_bank_bottom_address","drawee_address","document_enclosed", "consignee_address","consignor_address", "coo_issuer_address", "description_of_goods", "marks_and_no_of_packages"]:
                                for i in range(l1):
                                    new_row = row.copy()
                                    actual_value = actual[i]
                                    ...
                                    print(key)
                                    #def filter_prediction(new_row, actual_value, predicted, to_do, key, flag01):
                                    le2 = len(predicted_label)
                                    flag= True
                                    for j in range(le2):
                                        print('actual_value', actual_value[0].lower())
                                        print(predicted) 
                                        print(new_row) 
                                        if actual_value[0].lower() == predicted_label[j][0].lower() and flag==True:
                                            print("**********",new_row)
                                            print(actual_value[0].lower())
                                            print('entered into filter +++++++++++++++++===')
                                            flag= False                                   # consider one prediction 
                                    else:
                                        if flag == True: 
                                            #if key not in ["drawee_bank_address", "drawer_bank_address", "drawer_bank_bottom_address","drawee_address","document_enclosed"]:
                                            print("else executed")
                                            # exit('+++++++++++++')
                                            # print(new_row)
                                            new_row.append(actual_value[0])
                                            ...
                                            new_row.append(actual_value[1])
                                            data.append(new_row)
                                        #if  row[0]=='Covering_Schedule_30_page_3.png' and row[1]=="drawer_bank_address":
                                            print(new_row)
                                            #exit()  
                            else:'''
                            ...
                            if key in ["drawee_bank_address", "drawer_bank_address","drawee_address","drawer_address", "document_enclosed", "consignee_address","consignor_address", "coo_issuer_address", "description_of_goods", "marks_and_no_of_packages", 'notify_party_name','dimension', 'consignee_addres', 'carrier_country', 'carrier_name', 'agent_country','agent_name']:
                                act_val = ''
                                pre_val = ''
                                print(actual)
                                for i in range(len(actual)):
                                ...
                                del new_row1
                            if key in ["drawee_bank_address", 'agent_country', 'carrier_country', "drawer_bank_address","drawee_address","nostro_bank_address", "drawer_address", "consignee_address", "consignor_address", "coo_issuer_address", 'consignee_addres']:
                                actual = labels[key]
                                predicted_label = predicted[key]
                                act_val_address = ''
                                pre_val_address = ''
