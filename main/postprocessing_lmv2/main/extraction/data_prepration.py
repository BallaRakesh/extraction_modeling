...
def get_ocr_vision_api(image_path):
    os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = "/home/ntlpt19/Downloads/Trade_finance_imp_stage_2/extraction/oval-heuristic-387906-1815ed6ee296.json"
                    ...
                    for t in word_coordinates:
                        try:
                            ...
                            if get_intersection_percentage(data, t) >= 0.50:
                                # exit('+++++++++')
                                # print(get_intersection_percentage(internal_data, t))
                                t['label'] = data['label']
...
for file in labelled_files:
	if file in img_files:
		...
		if os.path.exists(os.path.join(images_path, file + ".png")):
			...
			for l in label:
				l = l.split()
				if (len(l) > int(configur['PARAMS']['nill_val'])) and (
						int(l[0]) != int(configur['PARAMS']['label_length'])):
