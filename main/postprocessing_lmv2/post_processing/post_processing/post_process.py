...
from pre_mapping import PredictionKeyMapping, ParentKeyMapping, OverLappingKeys, \
    transport_fields, weights_fields, amount_fields, currency_fields, \
        bottom_value, top_value, miscellenous_fields, number_fields
...
def final_check_actual_prediction(new_row1, actual, predicted_label, ratio_, data):
    ...
    for i in range(len(actual)):
        actual_value = actual_value + " " + actual[i][0]
    for j in range(len(predicted_label)):
        predicted_value = predicted_value+ " " + predicted_label[j][0]
        bbox = predicted_label[j][1]
        try:
            model_confidence = predicted_label[j][2]
        except:
            model_confidence = 0.9
    accuracy = fuzz.ratio(str(actual_value).lower().replace(' ', ''), str(predicted_value).lower().replace(' ', ''))
...
def filter_prediction(new_row, actual_value, predicted, to_do, data, ratio_, matching_flag):
    l2 = len(predicted)
    print(predicted)
    ...
    for j in range(l2):
        ...
        if fuzz.ratio(str(actual).lower().replace(' ', ''), str(pred).lower().replace(' ', '')) >= ratio_ and flag==True:
            print('entered into filter +++++++++++++++++===')
            flag= False                                   # consider one prediction 
            new_row.append(actual_value[0])
            new_row.append(predicted[j][0])
            accuracy = fuzz.ratio(str(actual).lower().replace(' ', ''), str(pred).lower().replace(' ', ''))
...
def final_overall_analysis(path1, path2, folder_path, doc_code, category_name):
    ...
    # Saving the CSV with proper line breaks
    with open(save_path_1, mode='w', newline='') as file:
...
def final_report(csv01, folder_path, doc_code, category_name):
    ...
    data_dict = report.to_dict(orient='records')  # 'records' will give a list of dictionaries   
    # Create DataFrame
    data_dict_df = pd.DataFrame(data_dict)
    # Save to CSV
    # data_dict_df.to_csv('output.csv', index=True)
    data_dict_df.to_csv(file_path_csv2)
    # txt file generation
...
def get_fuzz_accuracy(row, fuzzy_ratio):
    fuzzy_accuracy = fuzz.ratio(str(row[2]).lower().replace(' ', ''), str(row[3]).lower().replace(' ', ''))
...
def pre_handle_in_ic(key, labels, predicted, row, data):
    if key in labels and key in predicted:
        ...
        if key in ['sum_insured_currency', 'sum_insured_amount']:
            # actual = [[actual[0][i], actual[1]] for i in range(len(actual[0]))]
            print(predicted_label)
            # predicted_label = [[predicted_label[0][i], predicted_label[1], predicted_label[2]] for i in range(len(predicted_label[0]))]
            print(actual)
            print(predicted_label)
            # for i in range(len(actual)):
...
def handle_numeric_fields(row, key, index1=None): 
    if index1:
        ...
        if key in weights_fields:
            row = mapping_prediction_to_actual(row)
...
def handle_single_word_fields(key, row, incoterm_list, index1=None):
    if index1:
        ...
        if key == "means_of_transport":
            row[index1] = get_words(row[index1],['road', 'sea', 'air', 'ship', 'Post', 'Rail'])
            row = mapping_actual_to_prediction(row)
        ...
        if key == "usance_tenor":
            row[index1] = remove_words(row[index1],['DAYS'])
        if key in ["original", "bol_original_or_copy"]:
            row[index1] = get_words(row[index1],['original', 'org', 'copy', 'cpy'])
        if key == 'bol_original_number':
            row[index1] = handle_original_number(row[index1])
    return row
...
def handle_single_line_fields(key, row, index1=None):
    if index1:
        ...
        if key =="vessel_name":										
            row[index1] = remove_words(row[index1],['BY'])
    return row
        ...
        if key not in label_category.remove_fields:
            if key in labels and key in predicted:
                if len(labels[key]) == 1 and len(predicted[key]) == 1:
                    ...
                    row.append(clean_text_sep(str(predicted[key][0][0])))
                    print("before pp >>>>>>>", row)
                    ...
                    if key in miscellenous_fields:
                        ratio_ = config_['FuzzyRatio']['miscellenous_fields']
                    ratio_ = int(ratio_)
                    ...
                    for i in range(l2):
                        ...
                        predicted_label[i] = clean_text_sep(predicted_label[i],  index1=0)
                    for i in range(l2):
                        if key in label_category.numeric_fields:
                            ratio_ = config_['FuzzyRatio']['numeric_fields']
                    ...
                    for i in range(l1):
                        ...
                        print(f'the actual value is: {actual_value}')
                        print(f'the predicted value is: {predicted_label}')
                        to_do, matching_flag = filter_prediction(new_row, actual_value, predicted_label, to_do, data, ratio_, matching_flag)
                    print(to_do)
                    if not matching_flag:
                        new_row1 = row.copy()
                        final_check_actual_prediction(new_row1, actual, predicted_label, ratio_, data)
