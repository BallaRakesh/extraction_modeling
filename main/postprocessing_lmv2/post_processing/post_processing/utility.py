...
def mapping_prediction_to_actual(row):
	if row[2].lower() in row[3].lower():
		row[3] = row[2]
	return row

def mapping_actual_to_prediction(row):
	if row[3].lower() in row[2].lower():
		row[2] = row[3]
	return row
...
def handle_original_number(input_str):
...
def wrapping_up(key, pre_act_dict, flag):
	...
	if flag == 'actual':
		...
		pre_act_dict[key] = [[sum_insured_currency_lis[i],bbox] for i in range(len(sum_insured_currency_lis))]
	elif flag == 'pred':
		# pre_act_dict[key]=[sum_insured_currency_lis, bbox, confi]
		pre_act_dict[key] = [[sum_insured_currency_lis[i], bbox, confi] for i in range(len(sum_insured_currency_lis))]
...
def currency_amount_segregation(pred_actual_list, data_json, child_key1, child_key2, parent_field = None, prediction_flag = None):
		...
		if parent_field in pred_actual_list:
			for i in range(len(data_json[parent_field])):
				...
				if len(act_amount)>0 and child_key2 in pred_actual_list:
					if prediction_flag:
						data_json[child_key2].append([act_amount, data_json[parent_field][i][1], data_json[parent_field][i][2]])
					else:
						data_json[child_key2].append([act_amount, data_json[parent_field][i][1]])
					# data_json[child_key2].append(data_json[parent_field][i][1])
				else:
					if len(act_amount)>0:
						if prediction_flag:
							data_json[child_key2]=[[act_amount, data_json[parent_field][i][1], data_json[parent_field][i][2]]]
						else:
							data_json[child_key2]=[[act_amount, data_json[parent_field][i][1]]]
				if len(act_currency)>0 and child_key1 in pred_actual_list:
					if prediction_flag:
						data_json[child_key1].append([act_currency, data_json[parent_field][i][1], data_json[parent_field][i][2]])
					else:
						data_json[child_key1].append([act_currency, data_json[parent_field][i][1]])
					# data_json[child_key2].append(data_json[parent_field][i][1])
				else:
					if len(act_currency)>0:
						if prediction_flag:
							data_json[child_key1]=[[act_currency, data_json[parent_field][i][1], data_json[parent_field][i][2]]]

						else:
							data_json[child_key1]=[[act_currency, data_json[parent_field][i][1]]]
			print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
			print(data_json)
	if child_key2 in pred_actual_list:
		...
		for i in range(len(data_json[child_key2])):
			...
			if len(act_currency)>0 and child_key1 in pred_actual_list:
				if prediction_flag:
					data_json[child_key1].append([act_currency, data_json[child_key2][i][1], data_json[child_key2][i][2]])
				else:
					data_json[child_key1].append([act_currency, data_json[child_key2][i][1]])

				# data_json[child_key1].append(data_json[parent_field][i][1])
			else:
				if len(act_currency)>0:
					if prediction_flag:
						data_json[child_key1]=[[act_currency, data_json[child_key2][i][1], data_json[child_key2][i][2]]]

					else:
						data_json[child_key1]=[[act_currency, data_json[child_key2][i][1]]]


			if len(act_amount)>0: #and child_key2 in pred_actual_list:
				data_json[child_key2][i][0] = act_amount   
	print(data_json)
	if child_key1 in pred_actual_list:
		for i in range(len(data_json[child_key1])):
			...
			if len(act_amount)>0 and child_key2 in pred_actual_list:
				if prediction_flag:
					data_json[child_key2].append([act_amount, data_json[child_key1][i][1], data_json[child_key1][i][2]])
				else:
					data_json[child_key2].append([act_amount, data_json[child_key1][i][1]])
				# data_json[child_key1].append(data_json[parent_field][i][1])
			else:
				if len(act_amount)>0:
					if prediction_flag:
						data_json[child_key2]=[[act_amount, data_json[child_key1][i][1], data_json[child_key1][i][2]]]

					else:
						data_json[child_key2]=[[act_amount, data_json[child_key1][i][1]]]


			if len(act_currency)>0: #and child_key2 in pred_actual_list:
				data_json[child_key1][i][0] = act_currency
