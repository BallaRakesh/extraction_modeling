...
def set_basic_config_for_logging(filename: str = None, folder_path=None):
    """
    Set the basic config for logging a Python program.   
    :param filename: Name of the log file (without extension).
    :param folder_path: Path to the folder where the log file should be stored.
    :return: None   
    """
    if folder_path is None:
        folder_path = os.getcwd()  # Default to current working directory
    if filename is None:
        filename = "logfile"  # Default log file name
    ...
    logging.basicConfig(
        ...
        filemode='w',
        level=logging.INFO  # Optional: Set log level
    )
    print(f"Logging configured. Log file path: {log_file_path}")
...
def create_input_dict(
    ...
    """
    Custom function to prepare inputs for LayoutLMv2, handling long sequences.

    Args:
        words (list): List of words from the document.
        bboxes (list): Corresponding bounding boxes for the words.
        image_path (str): Path to the image.
        tokenizer: Tokenizer for tokenizing words.
        max_seq_length (int): Maximum sequence length (default: 512).
        img_size (int): Image size for resizing (default: 224).
        device (str): Device for tensor creation (default: "cpu").

    Returns:
        dict: Encoded input dictionary with split chunks ready for the model.
    """
    # Handle empty inputs
    if not words or not bboxes:
        words = ["[PAD]"]
        bboxes = [[0, 0, 0, 0]]
        print("Warning: Empty words or bboxes provided. Using default padding.")

    # Initialize token and bounding box lists
    token_list = [tokenizer.cls_token_id]
    bbox_list = [[0, 0, 0, 0]]  # CLS token bbox
    ...
    # Split into chunks of size 510 (to leave space for CLS and SEP)
    token_chunks = [token_list[i:i + 510] for i in range(0, len(token_list), 510)]
    bbox_chunks = [bbox_list[i:i + 510] for i in range(0, len(bbox_list), 510)]

    # Add CLS and SEP to each chunk, and pad to max_seq_length
    input_id_chunks, bbox_chunks_padded, attention_mask_chunks, token_type_id_chunks = [], [], [], []
    for token_chunk, bbox_chunk in zip(token_chunks, bbox_chunks):
        # Add CLS and SEP
        token_chunk = [101] + token_chunk + [102]
        bbox_chunk = [[0, 0, 0, 0]] + bbox_chunk + [[0, 0, 0, 0]]

        # Padding
        pad_len = max_seq_length - len(token_chunk)
        input_ids = torch.tensor(token_chunk + [0] * pad_len, dtype=torch.long)
        bbox = torch.tensor(bbox_chunk + [[0, 0, 0, 0]] * pad_len, dtype=torch.long)
        attention_mask = torch.tensor([1] * len(token_chunk) + [0] * pad_len, dtype=torch.long)
        token_type_ids = torch.tensor([0] * max_seq_length, dtype=torch.long)

        # Append to lists
        input_id_chunks.append(input_ids)
        bbox_chunks_padded.append(bbox)
        attention_mask_chunks.append(attention_mask)
        token_type_id_chunks.append(token_type_ids)

    # Stack chunks
    input_ids = torch.stack(input_id_chunks)
    attention_mask = torch.stack(attention_mask_chunks)
    token_type_ids = torch.stack(token_type_id_chunks)
    bbox = torch.stack(bbox_chunks_padded)

    # Process image
    image = Image.open(image_path).convert("RGB")
    image = image.resize((img_size, img_size))
    image = torch.FloatTensor(np.array(image)).permute(2, 0, 1).unsqueeze(0)  # Add batch dimension
    image = image / 255.0  # Normalize to [0, 1]
    images = torch.stack([image.squeeze(0)] * len(input_id_chunks))  # Repeat for each chunk
...
def image_result(file, path01, model, processor, device, folder_path, label_path= None):
	result_path = f"{folder_path}/Results_{path01}/"
	# label_path = os.path.join(folder_path, "classes.txt")
	# label_path = '/home/data_science/geo_testing/COO_V3/CORD_DATA/classes.txt'
	ocr_path = os.path.join(folder_path, "OCR")
	...
	for i, image in enumerate(ImageSequence.Iterator(im)):
		...
		try:
			with open(os.path.join(ocr_path, file[:-4] + "_text.txt"), "r") as f:
				...
				f.close()
			'''with open(os.path.join(ocr_path, file[:-4] + "_text.txt"), 'r') as f:
				word_coordinates = f.read()
				word_coordinates = ast.literal_eval(word_coordinates) 
			f.close()'''
			...
			try:
				...
				print(os.path.join(ocr_path, file[:-4] + "_text.txt"))
				exit('OCR NOT AVAILABLE')
		...
		# exit()
		encoded_inputs = create_input_dict(words,bboxes,os.path.join(folder_path, path01, file),max_seq_length=512,img_size=224,device="cpu")
		################ with processor ################
		...
		for i, output in enumerate(outputs.logits):
			...
			# print(all_boxes)
			# print('>>>>>>>>>')
			# print(input_dict['input_ids'][i])
			for id in input_dict['input_ids'][i]:
				# all_text.append(processor.tokenizer.decode(id))
				print('$$$$$$$$$$$$$$$$$$', id)
				# id_int = id.item()
				all_text.append(tokenizer.convert_ids_to_tokens(id.item()))
