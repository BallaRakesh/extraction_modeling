...
def train_test_split(split_file, flag, img_files, ocr_file_suffix_to_save):
    if debug_mode:
            ...
            if os.path.exists(os.path.join(images_path, file + ".png")):
                ...
                if len(labelled_data) > int(parser['PARAMS']['nill_val']):
                    ...
                    try:
                        ...
                        cv2.imwrite(filename=f"{master_path}/debug/annotation_validation/token_bbox/{file}_token_bbox.png",
...
if __name__ == "__main__":
    ...
    names_of_labelled_files: List = [x.split(".txt")[0] for x in labelled_files][:10]
    ...
    # fixing the seed and shuffling the filenames
    random.seed(seed_val)
    random.shuffle(labelled_files)

    train_samples_imgs = names_of_labelled_files[:-int(ratio_val * len(names_of_labelled_files))]
    logger.info(f"Length of the train data is {len(train_samples_imgs)}")
    print(len(train_samples_imgs))
    test_samples_imgs = names_of_labelled_files[-int(ratio_val * len(names_of_labelled_files)):]
