...
for doc_code_ in doc_code_list:
    ...
    print(label_new)
    new_labels = list(set(all_labels))
    print(new_labels)
    print(len(new_labels))
    
    #define your labels here
    new_labels = ['S-transaction_date', 'S-beneficiary_bank_swift', 'S-unit', 'S-gross_weight', 'S-payment_terms', 'S-date_of_invoice', 'S-lc_ref_no', 'S-order_date', 'S-invoice_tax_amount', 'S-shipper_name', 'S-freight_charges', 'S-tolerance_of_quantity', 'O', 'S-transport_details', 'S-place_of_receipt', 'S-country_of_final_destination', 'S-notify_party_name', 'S-remitter_country', 'S-beneficiary_country', 'S-final_destination', 'S-pre-carriage-by', 'S-invoice_no', 'S-insurance_charges', 'S-cosignee_name', 'S-tenor_indicator', 'S-incoterm', 'S-original_or_copy', 'S-page_no', 'S-vessel_flight_no', 'S-beneficiary_bank_identifier', 'S-port_of_loading', 'S-awb_number', 'S-transaction_amonunt_value', 'S-shipper_address', 'S-goods_description', 'S-consignor_name', 'S-stamp', 'S-bill_of_lading_no', 'S-transhipment', 'S-invoice_due_date', 'S-indicator_type', 'S-importer_your_ref_no', 'S-rate_per_unit', 'S-beneficiary_bank_country', 'S-notify_party_country', 'S-beneficiary_drawer_exporter_seller_supplier_tin', 'S-signature', 'S-invoice_amount', 'S-lc_date', 'S-advance_amount', 'S-beneficiary_address', 'S-signature_of_the_issuer', 'S-beneficiary_account_number', 'S-signed_by', 'S-from', 'S-exporter_our_ref_no', 'S-remitter_drawee_applicant_importer_buyer_name', 'S-packaging_description', 'S-notify_party_address', 'S-hs_code_no', 'S-beneficiary_iban_number', 'S-beneficiary_drawer_exporter_seller_supplier_name', 'S-beneficiary_bank_name', 'S-delivery_terms', 'S-total_quantity_of_goods', 'S-dimension', 'S-invoice_amount_in_words', 'S-invoice_discount_ccy', 'S-declaration_by_issuer', 'S-port_of_discharge', 'S-total_pages', 'S-usance_tenor', 'S-amount_due', 'S-purchase_order_no_and_agreement_ref_no', 'S-vessel_name', 'S-indicator_date', 'S-invoice_tax_ccy', 'S-mode_of_despatch', 'S-net_weight', 'S-awb_date', 'S-country_of_origin_of_goods', 'S-invoice_currency', 'S-bill_of_lading_date', 'S-shipment_date', 'S-consignee_address', 'S-invoice_discount_amount', 'S-transaction_currency', 'S-consignor_country', 'S-to', 'S-consignor_address', 'S-remitter_address', 'S-drawer_ref_no', 'S-tenor_type', 'S-presentation_period']
    # new_labels = ['S-signature', 'S-gross_weight', 'S-freight_collect_at', 'S-shipper_name', 'S-notify_party_address', 'S-bol_original_or_copy', 'S-mode_of_transport', 'S-voyage_number', 'S-measurement', 'S-goods_marks_and_nos', 'S-bol_original_number', 'S-container_number', 'S-dimension', 'S-pre_carriage_by', 'S-bill_of_lading_number', 'S-notify_party_name', 'S-port_of_discharge', 'S-final_destination', 'S-net_weight', 'S-signed_by_master', 'S-shipped_onboard_date', 'S-lc_date', 'S-lc_ref_number', 'S-signed_by_carrier', 'S-place_of_issue', 'S-bill_of_lading_issue_date', 'S-signed_by_agent', 'S-total_original_no', 'S-vessel_name', 'S-carrier_country', 'S-gross_quantity', 'S-transaction_date', 'S-consignee_address', 'O', 'S-shipper_address', 'S-page_no', 'S-goods_description', 'S-freight_collect_or_prepaid', 'S-agent_country', 'S-stamp', 'S-consignee_name', 'S-country_of_origin', 'S-port_of_loading', 'S-carrier_name', 'S-place_of_receipt', 'S-agent_name']
    
    
    #awb final keys 
    # new_labels = need to assign the prepared list of lables (AWB_classes > BOL_classes)
    ...
    id2label = {idx: label for idx, label in enumerate(new_labels)}
    print(label2id)
    print(id2label)
    ...
    model_path = '/New_Volume/Rakesh/DATA_LMV2/LMV2_BASE_itter_2/CI_PI/Best_Model'
    ...
    model = LayoutLMv2ForTokenClassification.from_pretrained(
        pretrained_model_name_or_path=os.path.join(model_path, 'pytorch_model.bin'),
        config=os.path.join(model_path, 'config.json'), num_labels=len(new_labels), ignore_mismatched_sizes = True)
    ...
    for epoch in range(num_train_epochs):
        ...
        # labels = list(set(all_labels))
        val_result, class_report = results_test(preds_val, out_label_ids, new_labels)
    ...
    # labels = list(set(all_labels))
    val_result, class_report = results_train(preds_val, out_label_ids, new_labels)
