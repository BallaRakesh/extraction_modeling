...
def calculatte_horizontal_dist(bb1, bb2):
    return abs(bb1[2] - bb2[0])



# finds and merges the model-output for text result, OCR confidence and Model confidence
	...
	for key in list(data.keys()):
		...
		if length > 1:
			...
			while i in range(length - 1):
				...
				# min_dist_horizontal = calculatte_horizontal_dist(bb1, bb2)
				...
				if inter_percentage != 1.0:
					if (min_dist_horizontal <= eps_horizontal or IOU_horizontal > 0 or inter_percentage > 0) and \
...
if __name__ == '__main__':
	...
	for doc_code_ in doc_code_list:
		...
		image_files = [x.split(".txt")[0] for x in image_files]
