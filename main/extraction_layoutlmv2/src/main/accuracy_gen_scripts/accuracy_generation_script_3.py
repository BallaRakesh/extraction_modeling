...
from src.main.extraction.accuracy_generation_script_2_latest import extract_country_from_text
import re
...
if __name__ == '__main__':
    ...
    print("Number of images in master internal_data", len(glob.glob(data_path + "/*.png")))

    # exit("+++++++++++")

    # validating the number of files coming from both these folders
    print(f"number of files coming from the Master internal_data folder: {len(data_files)}")
                ...
                if key in labels and key in predicted:
                    if len(labels[key]) == 1 and len(predicted[key]) == 1:
                        ...
                        if key not in ['sum_insured_amount']:
                            ...
                            if key not in ['invoice_date', 'csh_due_date', 'transaction_date', 'lc_date',
                                           'csh_presentation_date', 'expiry_date', 'bill_of_lading_issue_date',
                                           'sail_on_or_about_to_date', 'date_of_invoice', 'tenor_indicator_date',
                                           'shipped_onboard_date', 'issue_date', 'awb_date', 'bill_of_lading_date']:
                                row[2], row[3] = remove_start_end_spl_char(row[2], row[3])
                            print(f'after remove spl chars: {row}')
                            # exit()

                            if key in ['invoice_date', 'csh_due_date', 'transaction_date', 'lc_date',
                                       'csh_presentation_date', 'expiry_date', 'bill_of_lading_issue_date',
                                       'sail_on_or_about_to_date', 'date_of_invoice', 'tenor_indicator_date',
                                       'shipped_onboard_date', 'issue_date', 'awb_date', 'bill_of_lading_date']:
                                """row[2]=row[2].strip()
                            ...
                            # removing port of loading and port of discharge text out of internal_data
                            if key in ['port_of_discharge', 'port_of_loading']:
                                if key == 'port_of_discharge':
                                    ...
                                    row[2] = remove_label_name(row[2], regex)
                                    row[3] = remove_label_name(row[3], regex)
                            if key in ["page_no", "original_Number", "to_place", "lc_ref_no", "lc_ref_number",
                                       "consignee_name"]:
                                regex = [r"^(?i)To\s*:", r"^(?i)(?:LC\s*NO\.?\s*|NO\.|no\.)",
                                         "(?i)(page|no\.|no|pago\s*no\.|pago|Page)", r"(?i)NOTIFY\s*:\s*"]
                            ...
                            if key in ["pre_carriage_by", "consignor_name", "declaration_by", "declaration",
                                       "mode_of_transport", "from_place"]:
                                ...
                                if key == "declaration_by" or key == "declaration":
                                    row[2] = remove_words(row[2], ["REMOVE", "FOR", "NAME", "OF", "THE", "AUTHORISED",
                                                                   "SIGNATORY", "SEAL"])
                                    row[3] = remove_words(row[3], ["REMOVE", "FOR", "NAME", "OF", "THE", "AUTHORISED",
                                                                   "SIGNATORY", "SEAL"])
                        ...
                        if key in ['carrier_country', "country_of_final_destination", "country_of_origin_of_goods",
                                   'agent_country', "country_of_origin_origin_of_goods"]:
                            for i in range(l2):
                        ...
                        if key in ['port_of_discharge', 'port_of_loading']:
                            regex = ["([PDo])?(o)?(r)?(t)?(\s)?(o)?(f)?(\s)?([DO0])?(i)?(s)?(c)?(h)?(a)?(r)?(g)?(e)?",
                                     r"([PDo])?(o)?(r)?(t)?(\s)?(o)?(f)?(\s)?([IL1T\|])?(o)?(a)?(d)?(i)?(n)?(g)?"]
                            if key == 'port_of_discharge':
                                for i in range(l2):
                        ...
                        if key in ["page_no", "original_Number", "to_place", "lc_ref_no", "lc_ref_number",
                            ...
                            if key == 'page_no' or key == "original_Number":
                                for i in range(l2):
                                    actual_value = generic_page_no(
                                        remove_special_chars(remove_label_name(actual_value, regex[2])))
                                    predicted_label[i][0] = generic_page_no(
                                        remove_special_chars(remove_label_name(predicted_label[i][0], regex[2])))
                            elif key == 'consignee_name':
                                for i in range(l2):
                        ...
                        if key in ["pre_carriage_by", "consignor_name", "declaration_by", "declaration",
                            ...
                            if key == "declaration_by" or key == "declaration":
                                for i in range(l2):
                                    predicted_label[i][0] = remove_words(predicted_label[i][0],
                                                                         ["REMOVE", "FOR", "NAME", "OF", "THE",
                        ...
                        if key in [
                            ...
                            for i in range(l2):
                                if key == 'insurance_issuer_address':
                                    ...
                                    predicted_label[i][0] = remove_text_after_phrases(
                                        filter_address(predicted_label[i][0]))
    ...
    df.loc[(df["label_name"].isin(["sum_insured_currency_buyer", "sum_insured_currency_buyer"]) & (
        df["predicted"].str.contains("rs|usd|inr|eur", regex=True))), "Match/No_Match"] = 1
    print(df["Match/No_Match"].value_counts())
