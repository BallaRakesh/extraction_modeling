...
def filter_diclaration_by(new_row, actual_value, predicted):
    l2 = len(predicted)
    flag= True
    for j in range(l2):
        if fuzz.ratio(actual_value[0].lower(), predicted[j][0].lower())>85 and flag==True:
...
def filter_multi_prediction(new_row, actual_value, predicted):#, to_do):
    l2 = len(predicted)
    ...
    for j in range(l2):
        ...
        if actual_value.lower()== predicted[j].lower() and flag==True:
            print('entered into filter +++++++++++++++++===')
            flag= False                                   # consider one prediction 
            new_row.append(actual_value)
            new_row.append(predicted[j])
            accuracy = fuzz.ratio(str(new_row[2]).lower(), str(new_row[3]).lower())
            new_row.append(accuracy)
            if accuracy == 100:
                new_row.append(1)
...
def remove_words(text,words_to_remove_list):
    ...
    cleaned_text = cleaned_text.strip()
    return cleaned_text
# ['bill_exchange_date', 'invoice_date', 'indicator_date', 'invoice_due_date', 'lc_date']

def currency_amount_correction(key1, key2, actual_pred_list, labels_presdicted_json, flag1, flag2):
    if key1 in actual_pred_list:  #if flag2 =='currency'=> key1=beo_currency, key2 = beo_amount | if flag2 =='amount'=> key1=beo_amount, key2 = beo_currency
        for i in range(len(labels_presdicted_json[key1])):
            act_currency, act_amount = extract_currency_and_amount(str(labels_presdicted_json[key1][i][0]))
            if flag2 == 'amount':
                mapping_value1 = act_currency
                mapping_value2 = act_amount
            elif flag2 == 'currency':
                mapping_value1 = act_amount
                mapping_value2 = act_currency
            if len(mapping_value1)>0 and key2 in actual_pred_list:
                if flag1 == 'actaul':
                    labels_presdicted_json[key2].append([mapping_value1, labels_presdicted_json[key1][i][1]])
                elif flag1 == 'predicted':
                    labels_presdicted_json[key2].append([mapping_value1, labels_presdicted_json[key1][i][1], labels_presdicted_json[key1][i][2]])
                # labels['boe_amount'].append(labels['currency_amount'][i][1])
            else:
                if len(mapping_value1)>0:
                    if flag1 == 'actaul':
                        labels_presdicted_json[key2]=[[mapping_value1, labels_presdicted_json[key1][i][1]]]
                    elif flag1 == 'predicted':
                        labels_presdicted_json[key2]=[[mapping_value1, labels_presdicted_json[key1][i][1], labels_presdicted_json[key1][i][2]]]
            if len(mapping_value2)>0: #and 'boe_currency' in actual_list:
                labels_presdicted_json[key1][i][0] = mapping_value2   
    return labels_presdicted_json

def amount_in_words_filter(given_text):
    units = [
    ...
    seperator = ['and']
    final_text = ''
    for i in given_text.split(' '):
        if i.lower() in units or tens or scales or seperator:
...
if __name__ == '__main__':
    ...
    for file, predicted_files in zip(data_files, result_files):
        ...
        actual_curr = []
            ...
            # print(predicted) invoice_currency invoice_amount
        
        if 'boe_currency' in actual_list:
            labels = currency_amount_correction('boe_currency', 'boe_amount', actual_list, labels, 'actual', 'currency')
        if 'boe_currency' in pred_list:
            predicted = currency_amount_correction('boe_currency', 'boe_amount', pred_list, predicted, 'predicted', 'currency')
        if 'boe_amount' in actual_list:
            labels = currency_amount_correction('boe_amount', 'boe_currency', actual_list, labels, 'actual', 'amount')
        if 'boe_amount' in pred_list:
            predicted = currency_amount_correction('boe_amount', 'boe_currency', pred_list, predicted, 'predicted', 'amount')
        
        if 'invoice_currency' in actual_list:
            labels = currency_amount_correction('invoice_currency', 'invoice_amount', actual_list, labels, 'actual', 'currency')
        if 'invoice_currency' in pred_list:
            predicted = currency_amount_correction('invoice_currency', 'invoice_amount', pred_list, predicted, 'predicted', 'currency')
        if 'invoice_amount' in actual_list:
            labels = currency_amount_correction('invoice_amount', 'invoice_currency', actual_list, labels, 'actual', 'amount')
        if 'invoice_amount' in pred_list:
            predicted = currency_amount_correction('invoice_amount', 'invoice_currency', pred_list, predicted, 'predicted', 'amount')
            ...
            if key not in ['currency_amount', "doc_settlement_instructions","drawee_bank_country", "drawee_country", "drawer_country","drawer_bank_country", "document_enclosed", 'consignee_country', "signed_stamp", "certificate_stamped", "declaration_by", "signature"]:
                ...
                if key in labels and key in predicted:
                    if len(labels[key]) == 1 and len(predicted[key]) == 1:               
                        ...
                        if key in ['amount_in_words']:
                            row[2] = amount_in_words_filter(row[2])
                            row[3] = amount_in_words_filter(row[3])
                        if key =="pre_carriage_by"or key =="mode_of_transport" or key == 'means_of_transport' or key == 'tenor_type' or key == 'indicator_date':										
                            row[2] = remove_words(row[2],['BY',"EXPORT", 'freight', 'at', 'the'])
                            row[3] = remove_words(row[3],['BY',"EXPORT", 'freight', 'at', 'the'])
                        if key in ['bill_exchange_date', 'invoice_date', 'indicator_date', 'invoice_due_date', 'lc_date', "awb_date","bill_of_lading_date","bill_of_lading_issue_date","csh_presentation_date","date_of_invoice","end_date","expiry_date","indicator_date","invoice_date","invoice_due_date","issue_date","lc_date","sail_on_or_about_to_date","shipped_onboard_date","start_date","tenor_indicator_date","transaction_date"]: #actual
                        #if key in ['abc']:
                            row[2] = convert_to_common_format(row[2])
                            row[3] = convert_to_common_format(row[3])
                        if key not in ['bill_exchange_date', 'invoice_date', 'indicator_date', 'invoice_due_date', 'lc_date', "awb_date","bill_of_lading_date","bill_of_lading_issue_date","csh_presentation_date","date_of_invoice","end_date","expiry_date","indicator_date","invoice_date","invoice_due_date","issue_date","lc_date","sail_on_or_about_to_date","shipped_onboard_date","start_date","tenor_indicator_date","transaction_date"]:
                        ...
                        if key in ["drawee_bank_address", "drawer_bank_address", "drawer_bank_bottom_address",
                                    ...
                                    "doc_delivery_instruction",'invoice_amount', 'boe_amount', "boe_currency",'invoice_currency', "csh_presentation_date","csh_due_date", "consignee_address", "consignor_address", "coo_issuer_address", 'nostro_bank_address', 'consignor_address', 'address_of_assured', 'drawee_address', 'insurance_issuer_address', 'remitter_address', 'beneficiary_address', 'coo_issuer_address', 'notify_party_address', 'drawer_bank_address', 'consignee_address', 'drawer_address', 'drawee_bank_address', 'insurance_issuer_address_bottom', 'drawer_bank_bottom_address', 'shipper_address', 'claim_payable_by_address']:
                            #print("&&&&&&&&&&&&&&&")
                        ...
                        if key in ['bill_exchange_date', 'invoice_date', 'indicator_date', 'invoice_due_date', 'lc_date', "awb_date","bill_of_lading_date","bill_of_lading_issue_date","csh_presentation_date","date_of_invoice","end_date","expiry_date","indicator_date","invoice_date","invoice_due_date","issue_date","lc_date","sail_on_or_about_to_date","shipped_onboard_date","start_date","tenor_indicator_date","transaction_date"]:#, 'invoice_date']:
                            ...
                            actual_list = []
                            for i in range(l2):
                                pred_list.append(convert_to_common_format(predicted_label[i][0]))
                            for i in range(l1):
                                actual_list.append(convert_to_common_format(actual[i][0]))
                                
                            for k in range(len(actual_list)):
                                new_row = row.copy()
                                actual_value = actual_list[k]
                                filter_multi_prediction(new_row, actual_value, pred_list)
                        ...
                        if key not in ['invoice_no', 'amount_in_words', "drawee_bank_address", "drawer_bank_address","drawee_address","drawer_address", "document_enclosed", "consignee_address","consignor_address", "coo_issuer_address", "description_of_goods", "marks_and_no_of_packages", 'notify_party_name','dimension', 'consignee_addres', 'carrier_country', 'carrier_name', 'agent_country','agent_name','nostro_bank_address', 'consignor_address', 'address_of_assured', 'drawee_address', 'insurance_issuer_address', 'remitter_address', 'beneficiary_address', 'coo_issuer_address', 'notify_party_address', 'drawer_bank_address', 'consignee_address', 'drawer_address', 'drawee_bank_address', 'insurance_issuer_address_bottom', 'drawer_bank_bottom_address', 'shipper_address', 'claim_payable_by_address', 'to_place', 'means_of_transport', "awb_date","bill_of_lading_date","bill_of_lading_issue_date","csh_presentation_date","date_of_invoice","end_date","expiry_date","indicator_date","invoice_date","invoice_due_date","issue_date","lc_date","sail_on_or_about_to_date","shipped_onboard_date","start_date","tenor_indicator_date","transaction_date", "awb_date","bill_of_lading_date","bill_of_lading_issue_date","csh_presentation_date","date_of_invoice","end_date","expiry_date","indicator_date","invoice_date","invoice_due_date","issue_date","lc_date","sail_on_or_about_to_date","shipped_onboard_date","start_date","tenor_indicator_date","transaction_date","drawee_bank_address", 'consignee_addres', 'carrier_country', 'carrier_name', 'agent_country','agent_name', "drawer_address", "drawer_bank_address", "drawer_bank_bottom_address","drawee_address","document_enclosed", "consignee_address","consignor_address", "coo_issuer_address", "description_of_goods", "marks_and_no_of_packages","notify_party_name", 'dimension', 'to_place', 'means_of_transport', 'issue_date', 'date_of_invoice', 'lc_date', 'bill_of_lading_date','invoice_date','issue_date','nostro_bank_address', 'consignor_address', 'address_of_assured', 'drawee_address', 'insurance_issuer_address', 'remitter_address', 'beneficiary_address', 'coo_issuer_address', 'notify_party_address', 'drawer_bank_address', 'consignee_address', 'drawer_address', 'drawee_bank_address', 'insurance_issuer_address_bottom', 'drawer_bank_bottom_address', 'shipper_address', 'claim_payable_by_address']:
                            ...
                            for i in range(l1):
                                ...
                                if key in ['diclaration_by']:
                                    filter_diclaration_by(new_row, actual_value, predicted_label)
                                else:
                                    to_do = filter_prediction(new_row, actual_value, predicted_label, to_do, key, flag01)
                            # print(to_do)
                        if key in ['to_place', 'means_of_transport','amount_in_words', 'invoice_no']:
                            act_val = ''
                            pre_val = ''
                            print(actual)
