...
result_path = os.path.join(folder_path, "results_testing")
...
def sort_horizontal_previous(bboxes):
    same_line_sublists = []
    # Initialize variables to track the current line and the sublists
    current_line = []
    line_tolerance = 10
    # Create sublists of bounding boxes in the same line with horizontal tolerance
    for bbox in bboxes:
        if not current_line:
            current_line.append(bbox)
        else:
            prev_bbox = current_line[-1]
            if bbox[1] - line_tolerance <= prev_bbox[1] <= bbox[3] + line_tolerance:
                current_line.append(bbox)
            else:
                same_line_sublists.append(current_line)
                current_line = [bbox]

    # Append the last line to the list
    same_line_sublists.append(current_line)
    print(same_line_sublists)

    sorted_sublists = []

    for sublist in same_line_sublists:
        # Sort each sublist horizontally based on the x-coordinate (left side, x1)
        sublist.sort(key=lambda bbox: bbox[0])
        sorted_sublists.append(sublist)

    master_list = []
    for i in sorted_sublists:
        # print(i)
        for j in i:
            # print(j)
            master_list.append(j)

    # Print the sublists of bounding boxes in the same line
    print(master_list)
    return master_list



def sort_horizontal(bboxes):
...
def filter_using_scandwich(keys_list, final_result_set, bboxes):
    addition_dict = {}
...
def image_result(file, path01, model, processor, device):
    ...
    for i, image in enumerate(ImageSequence.Iterator(im)):
        ...
        bbox_list = []
        for key in list(final_result_set.keys()):
            all_values = final_result_set[key]    
            # print(all_values)
            for i in all_values:
                bbox_list.append(i[1])
        bboxes = sorted(bbox_list, key=lambda box: (box[1], box[3])) #vertically 
        ################
        sorted_bboxes = sort_horizontal(bboxes)
        for i in range(len(sorted_bboxes)-3):
            ...
            if abs(hori_sorted_bboxes[0][2]-hori_sorted_bboxes[1][0])<35 and abs(hori_sorted_bboxes[0][1]-hori_sorted_bboxes[1][1])<11:
            ...
            if flag1 == True and flag2 == True:
                ...
                print('>>>>>>>>>>>>>>>>>>>')
                # exit('???????????')
                final_result_set = filter_using_scandwich(keys_list, final_result_set, bboxes)
                
        print(final_result_set)
        # sorted_bboxes = sorted(bbox_list, key=lambda box: box[1])
        # print(bbox_list)
        # print('>>>>>>>>')
        # print(sorted_bboxes)
        # exit('++++++++++++++++++++')
