...
def custom_splitting():
    ...
    for split_mode, split_file in SPLIT_FILES.items():
        if split_file:
            ...
            annotation_json = [filename.replace('.png', '.json') for filename in img_files_list]
            if split_mode == 'train':
...
class DataSegmentation():
    def __init__(self, data_folder:str, train_folder:str, val_folder:str) -> None:
        ...
        image_files = [file for file in all_images_lst if file.endswith('.jpg') or file.endswith('.png')]
        ...
        if len(image_files) < 2 or len(annotation_files) < 2:
            ...
            try:
                ...
                #     image_file= image_file.split('.png')[0]
                #     shutil.copy2(os.path.join(all_images, image_file+'.png'), train_image)
                #     shutil.copy2(os.path.join(all_annotations, image_file+'.json'), train_annotation)
                # for image_file in val_image_files:
                #     image_file= image_file.split('.png')[0]
                #     shutil.copy2(os.path.join(all_images, image_file+'.png'), test_image)
                #     shutil.copy2(os.path.join(all_annotations, image_file+'.json'), test_annotation)
                
                for image_file in train_image_files:
                    image_file = image_file.split('.png')[0]
                    source_image = os.path.join(all_images, image_file + '.png')
    ...
    def __preprocess__(self, annot_path:str, file:str,data_folder:str, train_images_path:str):
        ...
        if len(data)>self.thresh:
            ...
            shutil.copy2(os.path.join(train_images_path,file+'.png'), os.path.join(f'{data_folder}/Images', file+'.png'))
    ...
    def __trainDataSeg__(self):
        ...
        image_files= [image.split('.png')[0] for image in image_files]
