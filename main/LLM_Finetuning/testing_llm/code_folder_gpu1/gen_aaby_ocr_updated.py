...
def get_config(image, file_name):
        ...
        try:
            with open(ABBY_OCR_ENGINE_DETAILS["ABBYY_CONFIG_PATH"], "r") as config:
             ...
             with open("/home/gpu1admin/rakesh/code/abby_config.json", "r") as config:
                config_file = json.load(config)
            ...
            for text_block in page[TEXTS]:
                for line in text_block[LINES]:
                    for word_data in line[WORDS]:
                        ...
                        width = position['r'] - position['l']
                        height = position['b'] - position['t']
                        x1 = position['l'] ; y1 = position['t']
                        x2 = position['r'] ; y2 = position['b']
...
def generate_ocr_string_and_word_coordinates(image, file_name):
        ...
        print("main response url :", "{}/{}".format(ABBY_OCR_ENGINE_DETAILS["ABBY_API"],ABBY_OCR_ENGINE_DETAILS["ABBY_PROCESSING_ENDPOINT"]))
        ...
        api_status_and_result_endpoint = "{}/{}".format(ABBY_OCR_ENGINE_DETAILS["ABBY_API"], ABBY_OCR_ENGINE_DETAILS["ABBYY_STATUS_ENDPOINT"].format(job_id))
        print("********\n", api_status_and_result_endpoint, "\n********")
        while True:
            # print("api_status_and_result_endpoint : ",api_status_and_result_endpoint)
            status_response = requests.get(api_status_and_result_endpoint, verify=False)
            # print("status_response : ", status_response.text)
        ...
        json_result = requests.get(
            "{}/{}".format(api_status_and_result_endpoint, ABBY_OCR_ENGINE_DETAILS["ABBY_JSON_RESULT_ENDPOINT"]), verify=False)
...
if __name__ == "__main__":
    ...
    for imgs in tqdm(os.listdir(image_path)):
        ...
        # if imgs not in ["411172_Invoice_page_0.png"]:
        #     continue
        ...
        if not os.path.exists(text_file_path):
            ...
            log_message = f"TIME TAKEN FOR CURRENT IMAGE {time.time() - img_start_time:.2f} seconds.\n"
            print(log_message)
            with open("ocr_generation_log.txt", "a") as log_file:  # 'a' mode appends to the file
                log_file.write(log_message)
            exit('OK')
