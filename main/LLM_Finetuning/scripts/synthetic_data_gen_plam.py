...
os.environ['PALM_API_KEY'] = 'AIzaSyBfkAGgG1xqlzkNeE_IxGDY5ATSwTEQpTw'
...
def decompose(ocr_data):
    relevant_schemas_ = 'bill of lading'
    query_ = ocr_data#'jdkvmksmk mjdm'#ocr_data
    prompt_template1 = f"""
        You are an expert at generating synthetic data for Trade Finance Domain
        Here are the documet name:
        {relevant_schemas_}

        Here's the sample OCR data:
        {query_}
        By looking at the above Example please generate the synthetic OCR data
    ...
    prompt_template = f"""
        ...
        \n
        By looking at the above Example plese generate the atleast five different drawer addresses 
        ...
        3) By looking at the above Example plese generate the new address
    ...
    decompose_template = PromptTemplate(
        input_variables = ['relevant_schemas_', 'query_'],
        template = prompt2
        )
    decompose_table = LLMChain(llm = palm_llm, prompt = decompose_template)
    query_responce = decompose_table.run(relevant_schemas_ = relevant_schemas_, query_ = query_)
    # print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
    # print(query_responce)
    return query_responce

content = open_text_file('/home/ntlpt19/LLM_training/TRAIN/BOL/all_text/Bill_Of_Landing_106_page_9_all_text.txt')
ocr_content  = eval(content).get('all_text', '')
ocr_data = """
