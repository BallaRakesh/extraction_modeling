...
from constants import keys_not_to_consider
...
def process_dict(input_dict, key_mapping, lables_train, threshold=20):
    """
...
if __name__=="__main__":
    ...
    for i, image in enumerate(tqdm(img_lst, desc= "master data prep...")):
        if image in label_lst and  os.path.exists(os.path.join(master_data_path, image+"_labels.txt")):
            ...
            gt_labels, lenth_ = process_dict(labels, key_mapping, lables_train)
            print(gt_labels)
            data.append({'file name': image, "text": text, 'ground truth': str(gt_labels), 'gt_len': lenth_})
        else:
