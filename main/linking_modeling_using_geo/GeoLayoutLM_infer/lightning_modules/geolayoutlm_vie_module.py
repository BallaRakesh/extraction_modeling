        ...
        if len(dump_dir) > 0:
            ...
            for token_idx in range(num_valid_tokens):
                if box_first_token_mask[token_idx]:
                    ...
                    # print(pr_str_i[valid_idx])
                    # predicted_class = pr_str_i[valid_idx].split("-")[-1]  # Extracts "QUESTION" from "B-QUESTION"
                    # if predicted_class in classes_result:
                    #     classes_result[predicted_class].append([count_flag])
            ...
            # idx_tagging_result = f'{img_name}_idx_tagging_result.txt'
            # with open(os.path.join(dump_dir, idx_tagging_result), 'w') as f_:
            #     f_.write(str(classes_result))
            # f_.close()
...
def do_eval_step_el(batch, head_outputs, loss, eval_kwargs, dump_dir=''):
    bio_class_names = eval_kwargs["bio_class_names"]
    pr_labels_helper = torch.argmax(head_outputs["logits4labeling"], -1)
    ...
    n_batch_gt_rel, n_batch_pr_rel, n_batch_correct_rel = eval_el_geo_batch(
        ...
        pr_labels_helper = pr_labels_helper
    )
...
def eval_el_geo_example(
    ...
    pred_labeling_ = pr_labels_helper[example_idx]
...
def parse_relations2_logic_for_reference(
    el_label,
    first_token_idxes,
    el_label_blk_mask,
    predicted_bio_labels,  # Changed from bio_labels to predicted_bio_labels
    bio_class_names,
    max_prob_as_father=False,
    max_prob_as_father_upperbound=False,
):
    ...
    if max_prob_as_father:
        ...
        threshold = (threshold - 1e-3) * (threshold > 0.5).float()

    for son_id in range(blk_num):
        if el_label_blk_mask[son_id, 0] == 0:
            break
        
        # Get predicted label for son
        son_bio_label = predicted_bio_labels[first_token_idxes[son_id]]
        son_class = bio_class_names[son_bio_label.item()]
        
        # Skip if son is "O" class
        if son_class == "O":
            continue
        ...
        for fthr_id in range(blk_num):
            ...
            # Get predicted label for father
            fthr_bio_label = predicted_bio_labels[first_token_idxes[fthr_id]]
            fthr_class = bio_class_names[fthr_bio_label.item()]
            
            # Only create relation if both entities are non-O class and satisfy probability threshold
            if fthr_class != "O" and (not max_prob_as_father or el_label[son_id, fthr_id] > threshold[son_id]):
                link_map_tuples.append((fthr_id, son_id))
...
def parse_relations2(
    ...
    for son_id in range(blk_num):
        ...
        # Get predicted label for son
        son_bio_label = pred_labeling[first_token_idxes[son_id]]
        son_class = bio_class_names[son_bio_label.item()]
        ...
        for fthr_id in range(blk_num):
            ...
            if el_label[son_id, fthr_id] == 0:
                continue
            # if bio_class_names[bio_labels[first_token_idxes[son_id]].item()] != "O" and \
            #     bio_class_names[bio_labels[first_token_idxes[fthr_id]].item()] != "O": #major change # needd to add this condtion in different way from prediction
            
            # Get predicted label for father
            fthr_bio_label = pred_labeling[first_token_idxes[fthr_id]]
            fthr_class = bio_class_names[fthr_bio_label.item()]
            ...
            # if not max_prob_as_father or el_label[son_id, fthr_id] > threshold[son_id]: # this need to under upper if condition 
            #     link_map_tuples.append((fthr_id, son_id))
