...
class DilatedBlock(nn.Module):
    ...
    def __init__(self, c, dilation, k, fuse="sum", shortcut=True):
        ...
        self.bachnorm_temp = nn.BatchNorm2d(c)
        
    def dilated_conv(self, x, dilation):
        act = self.dcv.act
        # print(Conv)
        if not hasattr(self.dcv, 'bn'):
            print('CAME here into the if block')
            print(Conv)
            print(self.dcv)
            bn = self.bachnorm_temp
            # raise ValueError(f"self.dcv has no 'bn' attribute. Type: {type(self.dcv)}")
        else:
            bn = self.dcv.bn
